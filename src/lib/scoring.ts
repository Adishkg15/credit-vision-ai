...
export interface AssessmentInputs {
  ...
  transactionConsistency: 1 | 2 | 3 | 4 | 5; // 1=erratic 5=very consistent
  digitalPaymentUsage: 1 | 2 | 3 | 4 | 5;
  failedTransactions: number;
  overdraftEvents: number;

  // Step 4 — Bill Payment (0=never, 1=often late, 2=sometimes late, 3=always on time, -1=NA)
  rentPayments: -1 | 0 | 1 | 2 | 3;
  utilityPayments: -1 | 0 | 1 | 2 | 3;
  telecomPayments: -1 | 0 | 1 | 2 | 3;
  subscriptionPayments: -1 | 0 | 1 | 2 | 3;

  // Step 5 — Employment Quality
...
export interface LoanRecommendation {
  ...
  interestRange: [number, number]; // % p.a.
  confidence: number;          // 0..100
  rationale: string;
}
...
/** Map bill payment value into score+confidence; -1 = NA (no penalty, lower confidence) */
function billScore(v: number): { score: number | null; confidence: number } {
  if (v === -1) return { score: null, confidence: 0 };
  // 0 never paid (no history), 1 often late, 2 sometimes late, 3 always on time
  if (v === 0) return { score: null, confidence: 0 }; // treat as missing info, not risk
...
export function computeAssessment(i: AssessmentInputs): AssessmentResult {
  ...
  if (income === 0) {
    ...
    finScore += clamp(surplusRatio * 100, -20, 50) + 20; // -20..70 → 0..90 range part
    finScore += clamp(savingsMonths * 6, 0, 30);          // 0..30
    finScore += clamp(i.emergencyFundMonths * 4, 0, 15);  // 0..15
  ...
  if (i.monthlyTransactions === 0 && i.avgBankBalance === 0) {
    ...
    bankScore += (i.transactionConsistency - 1) * 6;        // 0..24
    bankScore += (i.digitalPaymentUsage - 1) * 4;            // 0..16
  ...
  if (i.employmentType === "unemployed") {
  ...
  } else if (i.employmentType === "student" || i.employmentType === "fresher") {
    empScore = 55; // baseline — not penalised for lack of history
    empConf = 65;
    missing.push("Established employment history");
    empNotes.push(`${i.employmentType === "student" ? "Student" : "Fresher"}: scored on potential, not history.`);
    ...
    empScore += (i.employerStability - 1) * 5; // 0..20
    ...
    if (i.employmentType === "freelancer" || i.employmentType === "gig_worker") {
      empScore *= 0.9; // income volatility, not punishment
  ...
  // derived from banking consistency + bill regularity + low failures
  ...
  if (knownBills.length === 0) {
    ...
    behScore = 55; // neutral — absence of data, not negative
  } else {
  ...
  eduScore += (4 - i.institutionTier) * 5; // tier1=+15, tier2=+10, tier3=+5
  ...
  const confidenceScore = round(
    categories.reduce((s, c) => s + c.confidence * (c.weight / 100), 0)
  );

  // ---------- Risk + Eligibility ----------
  // Risk: low score => higher risk. Independent from confidence.
  ...
  const noCapacity = income <= 0 && i.savings <= 0 && i.employmentType === "unemployed";
  ...
  if (i.employmentType === "student" || i.employmentType === "fresher") insights.push("Profile evaluated on potential indicators (education, skills, banking habits) rather than credit history.");
  ...
  const baseDTI = 0.4; // max debt-to-income for monthly EMI
  ...
  function buildRec(product: string, opts: {
    minIncome?: number;
    multiplier: number;      // multiplier on monthly income for limit
    ...
    const tenureOk = (opts.minTenureMonths ?? 0) <= i.jobTenureMonths || ["student", "fresher"].includes(i.employmentType) && product === "Education Loan";

    if (noCapacity || income < minIncome || !tenureOk) {
      return {
        product,
        eligible: false,
        ...
        rationale: noCapacity
          ? "No reported income or savings — repayment capacity is zero."
          : income < minIncome
    ...
    // Capacity-derived amount, scaled by score and confidence
    const incomeBased = income * opts.multiplier * scoreFactor;
    const emiBased = affordableEMI * 36; // 3-year equivalent
    const raw = Math.min(incomeBased, emiBased * (0.5 + 0.5 * scoreFactor));
    const recommendedAmount = Math.max(0, Math.min(opts.maxAmount, Math.round(raw / 1000) * 1000));

    // Approval probability: blends score and confidence
    const approvalProbability = clamp(round((scoreFactor * 0.7 + confFactor * 0.3) * 100));

    // Interest range: better score → tighter, lower range
    ...
    return {
      ...
      interestRange: [lo, hi],
      confidence: confidenceScore,
      rationale: opts.eligibilityNote ?? `Derived from income ₹${income.toLocaleString("en-IN")}/mo, surplus ₹${Math.max(0, surplus).toLocaleString("en-IN")}/mo and overall score ${overallScore}.`,
    };
  }
  ...
  const recommendations: LoanRecommendation[] = [
    ...
    buildRec("Education Loan", { multiplier: 15, baseRate: 9, maxAmount: 4000000,
      eligibilityNote: i.employmentType === "student" || i.employmentType === "fresher"
  ...
  // If no capacity, force every recommendation to Not Eligible
  if (noCapacity) {
    for (const r of recommendations) {
      ...
      r.interestRange = [0, 0];
      r.rationale = "Not eligible — no income, savings, or employment indicates zero repayment capacity.";
    }
  }
